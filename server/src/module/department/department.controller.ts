...
export default class DepartmentController {
    ...
    public CreateDepartment = async(req, res, next) => {
        ...
        try{
            ...
            if(!verifyReq.pass){
                const err: any = new ErrorObject(verifyReq.message,ApiStatusCode.BadRequest, "20-creatDepartment- controller");
                return next(err);
            }
            const newDepartment = {
    ...
    public updateDepartment = async(req, res, next) => {
        ...
        try{
            const verifyReq = validateReqBody(req,departmentRequest)
            if(!verifyReq.pass){
                const err: any = new ErrorObject(verifyReq.message,ApiStatusCode.BadRequest, "54-updateDepartment- controller");
                return next(err);
            }
            const newDepartment = {
                displayName: req.body.displayName,
